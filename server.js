...
//when the app first launchers, make the public/uploaded=photos exists//
...
app.post('/create-animal', upload.single("photo"),ourCleanup, async (req,res)=> {
  if (req.file){
    ...
    await sharp(req.file.buffer).resize(844,456).jpg({quality:60}).toFile(path.join("public","uploaded-photos", photofilename))
...
function ourCleanup(req,res,next){
    ...
    req.cleanData ={
        ...
        species: sanitizeHTML(req.body.name.trim(),{allowedTags: [], allowedAttributes:{}}),
    }
    next()    
}
